...
Sys.WebForms.Menu = function (options) {
  ...
  if (this.element.tagName === 'DIV') {
    ...
    Sys.WebForms.Menu._domHelper.setFloat(
      this.element,
      this.container.rightToLeft ? 'right' : 'left'
    );
  } else {
  ...
  if (this.parentMenuItem && this.parentMenuItem.parentMenu) {
    ...
    if (!this.element.id) {
      this.element.id =
        (this.container.element.id || 'menu') +
        ':submenu:' +
        Sys.WebForms.Menu._elementObjectMapper._computedId;
    }
...
Sys.WebForms.MenuItem = function (parentMenu, listElement, topLevelMenuItem) {
  ...
  if (this._anchor) {
    ...
    if (subMenu) {
      ...
      if (!this.childMenu.isStatic()) {
        ...
        Sys.WebForms.Menu._domHelper.appendAttributeValue(
          this.element,
          'aria-haspopup',
          this.childMenu.element.id
        );
      }
    }
  }
  ...
  Sys.WebForms.Menu._domHelper.appendCssClass(
    listElement,
    parentMenu.displayMode
  );
  if (this._anchor) {
    Sys.WebForms.Menu._domHelper.appendCssClass(
      this._anchor,
  ...
  if (!this.container.disabled) {
    Sys.WebForms.Menu._domHelper.addEvent(
      this.element,
      'mouseover',
      Sys.WebForms.MenuItem._onmouseover
    );
    Sys.WebForms.Menu._domHelper.addEvent(
      this.element,
      'mouseout',
      Sys.WebForms.MenuItem._onmouseout
    );
  }
};
Sys.WebForms.MenuItem.prototype = {
  ...
  doDispose: function () {
    Sys.WebForms.Menu._domHelper.removeEvent(
      this.element,
      'mouseover',
      Sys.WebForms.MenuItem._onmouseover
    );
  ...
  highlight: function (highlighting) {
    if (highlighting) {
      this.applyUp(
        ...
        function (menuItem) {
          return (
            !menuItem.parentMenu.isStatic() &&
            menuItem.parentMenu.parentMenuItem
          );
        }
      );
  ...
  mouseout: function () {
    ...
    if (disappearAfter > -1) {
      this.container.pendingMouseoutId = window.setTimeout(function () {
        menuItem.hover(false);
      }, disappearAfter);
    }
  },
  ...
  navigateChild: function () {
    ...
    if (subMenu) {
      ...
      if (this.container.orientation === 'horizontal') {
        var nextItem =
          this.topLevelMenuItem.nextSibling ||
          this.topLevelMenuItem.parentMenu.firstChild();
...
Sys.WebForms.Menu._domHelper = {
  ...
  removeString: function (getString, setString, valueToRemove) {
    var currentValue = getString();
    if (currentValue) {
      var regex = this._regexes.getRegex(
        '(\\s|\\b)' + valueToRemove + '$|\\b' + valueToRemove + '\\s+'
      );
...
Sys.WebForms.Menu._elementObjectMapper = {
  ...
  map: function (element, theObject) {
    ...
    var objectId =
      element[this._mappingIdName] || element.id || '%' + ++this._computedId;
...
Sys.WebForms.Menu._keyboardMapping = new (function () {
  ...
  this.horizontal.contains =
    this.horizontalRtl.contains =
    this.vertical.contains =
    this.verticalRtl.contains =
      function (keycode) {
...
Sys.WebForms._MenuContainer = function (options) {
  ...
  this.rightToLeft =
    Sys.WebForms.Menu._domHelper.getElementDirection(this.element) === 'rtl';
  Sys.WebForms.Menu._elementObjectMapper.map(this.element, this);
    ...
    this.element.dispose = function () {
      if (menuContainer.element.dispose) {
        ...
        Sys.WebForms.Menu._domHelper.removeEvent(
          ...
          'focus',
          menuContainer._onfocus,
          true
        );
        Sys.WebForms.Menu._domHelper.removeEvent(
          menuContainer.menu.element,
          'keydown',
          menuContainer._onkeydown
        );
...
Sys.WebForms._MenuContainer.prototype = {
  ...
  _onfocus: function (e) {
    ...
    if (event.srcElement && this) {
      if (
        Sys.WebForms.Menu._domHelper.contains(this.element, event.srcElement)
      ) {
        if (!this.focused) {
          this.focus();
        }
