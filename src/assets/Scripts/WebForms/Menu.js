...
function Menu_Focus(item) {
  if (item && item.focus) {
    ...
    if (
      pos.y + pos.height >
      posParent.y + parentContainer.offset + parentContainer.clippedHeight
    ) {
      delta =
        pos.y +
        pos.height -
...
function Menu_HideItems(items) {
  ...
  if (
    !items ||
    (typeof items.tagName == 'undefined' && items instanceof Event)
  ) {
...
function Menu_HoverDisabled(item) {
  ...
  var data = Menu_GetData(item);
  if (!data) return;
  node = WebForm_GetElementByTagName(node, 'table').rows[0].cells[0]
    .childNodes[0];
...
function PopOut_Position(panel, hideScrollers) {
  ...
  var panelHeight =
    typeof panel.physicalHeight != 'undefined' && panel.physicalHeight != null
  ...
  WebForm_SetElementHeight(
    overflowElement,
    panelHeight + relCoordinates.y + parseInt(panel.y ? panel.y : 0)
  );
  ...
  if (typeof position == 'undefined' || position == null || position == '') {
    position =
      WebForm_GetElementDir(rel) == 'rtl' ? 'middleleft' : 'middleright';
  }
  position = position.toLowerCase();
  var y =
    relCoordinates.y +
    parseInt(panel.y ? panel.y : 0) -
  ...
  var panelY =
    typeof panel.originY != 'undefined' && panel.originY != null
      ? panel.originY
  ...
  if (position.indexOf('left') != -1) {
    ...
    overflow =
      x + panelParentCoordinates.x + panelCoordinates.width - rightWindowBorder;
    if (overflow > 0) {
      if (
        position.indexOf('bottom') == -1 &&
        relCoordinates.x > panelCoordinates.width
      ) {
...
function PopOut_SetPanelHeight(element, height, doNotClip) {
  if (element && element.style) {
    ...
    WebForm_SetElementHeight(
      element,
      height - (element.clientTop ? 2 * element.clientTop : 0)
    );
...
function PopOut_Show(panelId, hideScrollers, data) {
  ...
  if (panel && panel.tagName.toLowerCase() == 'div') {
    ...
    if (isIE && data) {
      ...
      if (
        panel.currentStyle &&
        panel.currentStyle.zIndex &&
        panel.currentStyle.zIndex != 'auto'
      ) {
    ...
    if (up && dn) {
      if (panel.offset && panel.offset > 0) {
        ...
        if (panel.clientWidth) {
          WebForm_SetElementWidth(
            up,
            panel.clientWidth - (up.clientLeft ? 2 * up.clientLeft : 0)
          );
        }
      ...
      if (panel.offset + panel.clippedHeight + 2 <= panel.physicalHeight) {
        ...
        if (panel.clientWidth) {
          WebForm_SetElementWidth(
            dn,
            panel.clientWidth - (dn.clientLeft ? 2 * dn.clientLeft : 0)
          );
        }
        WebForm_SetElementY(
          dn,
