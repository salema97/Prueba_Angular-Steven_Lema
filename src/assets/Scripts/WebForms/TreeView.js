...
function TreeView_GetNodeText(node) {
  ...
  if (trNode.childNodes[trNode.childNodes.length - 1].getElementsByTagName) {
    outerNodes =
      trNode.childNodes[trNode.childNodes.length - 1].getElementsByTagName('A');
    if (!outerNodes || outerNodes.length == 0) {
      outerNodes =
        trNode.childNodes[trNode.childNodes.length - 1].getElementsByTagName(
          'SPAN'
        );
    }
  }
  var textNode =
    outerNodes && outerNodes.length > 0
...
function TreeView_PopulateNode(
  data,
  index,
  node,
  selectNode,
...
function TreeView_ProcessNodeData(result, context) {
  ...
  if (result.length > 0) {
    ...
    var newExpandState = result.substring(
      context.data.lastIndex.length + 1,
      ci
    );
    ...
    if (__nonMSDOMBrowser) {
      ...
      newChildren = null;
      if (
        typeof table.nextSibling == 'undefined' ||
    ...
    if (typeof newChildren != 'undefined' && newChildren != null) {
      ...
      treeNode.href = document.getElementById
        ...
        : 'javascript:TreeView_ToggleNode(' +
          ...
          ')';
      if (
        typeof context.selectNode != 'undefined' &&
        context.selectNode != null &&
        context.selectNode.href &&
        context.selectNode.href.indexOf(
      ...
      if (
        typeof context.selectImageNode != 'undefined' &&
        context.selectImageNode != null &&
        context.selectNode.href &&
        context.selectImageNode.href.indexOf(
          'javascript:TreeView_PopulateNode',
          0
        ) == 0
      ) {
    ...
    var img = treeNode.childNodes
      ? treeNode.childNodes[0]
      : treeNode.children[0];
...
function TreeView_ToggleNode(data, index, node, lineType, children) {
  ...
  data.expandState.value =
    data.expandState.value.substring(0, index) +
    newExpandState +
