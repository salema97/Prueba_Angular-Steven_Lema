...
function __wpTranslateOffset(
  ...
  while (
    typeof offsetElement != 'undefined' &&
    offsetElement != null &&
    offsetElement != relativeToElement
  ) {
...
function WebPart(
  ...
  this.title =
    typeof webPartTitleElement != 'undefined' && webPartTitleElement != null
      ? webPartTitleElement.innerText
      : '';
  webPartElement.__webPart = this;
  if (
    typeof webPartTitleElement != 'undefined' &&
    webPartTitleElement != null
  ) {
...
function WebPart_OnDragStart() {
  ...
  if (
    typeof webPartElement.__webPart == 'undefined' ||
    webPartElement.__webPart == null
  ) {
    currentEvent.returnValue = false;
    currentEvent.cancelBubble = true;
    return;
...
function Zone(
  ...
  this.savedBorderColor =
    webPartTable != null ? webPartTable.style.borderColor : null;
  this.dropCueElements = new Array();
  if (webPartTable != null) {
    if (isVertical) {
      for (i = 0; i < webPartTable.rows.length; i += 2) {
        this.dropCueElements[i / 2] =
...
function Zone_ToggleDropCues(show, index, ignoreOutline) {
  if (ignoreOutline == false) {
    this.webPartTable.style.borderColor = show
      ? this.highlightColor
      : this.savedBorderColor;
  }
  if (index == -1) {
...
function Zone_GetWebPartIndex(location) {
  ...
  var y = location.y;
  if (
    x < this.webPartTableLeft ||
    x > this.webPartTableRight ||
    y < this.webPartTableTop ||
    y > this.webPartTableBottom
  ) {
    return -1;
  }
...
function Zone_UpdatePosition() {
  ...
  this.webPartTableRight =
    this.webPartTable != null
      ? topLeft.x + this.webPartTable.offsetWidth
      : topLeft.x;
  this.webPartTableBottom =
    this.webPartTable != null
      ? topLeft.y + this.webPartTable.offsetHeight
      : topLeft.y;
  for (var i = 0; i < this.webParts.length; i++) {
...
function WebPartMenu(menuLabelElement, menuDropDownElement, menuElement) {
  ...
  if (
    typeof this.menuDropDownElement != 'undefined' &&
    this.menuDropDownElement != null
  ) {
...
function WebPartMenu_Show() {
  ...
  var menuHTML =
    '<html><head><style>' +
    'a.menuItem, a.menuItem:Link { display: block; padding: 1px; text-decoration: none; ' +
    this.itemStyle +
    ' }' +
    'a.menuItem:Hover { ' +
    this.itemHoverStyle +
  ...
  if (
    this.menuElement.innerHTML.indexOf(
  ...
  this.popup.show(
    0,
    this.menuLabelElement.offsetHeight,
    width,
    height,
...
function WebPartMenu_Hover() {
  if (this.labelHoverClassName != '') {
    this.menuLabelElement.className =
...
function WebPartManager_AddZone(
  zoneElement,
  uniqueID,
  isVertical,
  allowLayoutChange,
...
function WebPartManager_IsDragDropEnabled() {
  return (
    typeof this.overlayContainerElement != 'undefined' &&
    this.overlayContainerElement != null
  );
}
function WebPartManager_DragDrop() {
  if (
    typeof this.draggedWebPart != 'undefined' &&
    this.draggedWebPart != null
  ) {
...
function WebPartManager_CompleteWebPartDragDrop() {
  ...
  this.dragState = null;
  if (
    typeof dragState.dropZoneElement != 'undefined' &&
  ...
  if (
    ...
    if (
      currentZone != dragState.dropZoneElement.__zone ||
      ...
      var eventArgument =
        'Drag:' + dragState.webPartElement.id + ':' + dragState.dropIndex;
...
function WebPartManager_ProcessWebPartDragEnter() {
  ...
  if (typeof dragState != 'undefined' && dragState != null) {
    ...
    if (
      typeof newDropZoneElement.__zone == 'undefined' ||
      newDropZoneElement.__zone == null ||
    ...
    var newDropIndex = -1;
    if (
      typeof newDropZoneElement != 'undefined' &&
      newDropZoneElement != null
    ) {
      newDropIndex = newDropZoneElement.__zone.GetWebPartIndex(
        __wpGetPageEventLocation(currentEvent, false)
      );
      if (newDropIndex == -1) {
        newDropZoneElement = null;
      }
...
function WebPartManager_ProcessWebPartDragOver() {
  ...
  if (
    ...
    if (
      typeof dropZoneElement != 'undefined' &&
      dropZoneElement != null &&
      dropZoneElement.__zone.allowDrop == false
    ) {
      dropZoneElement = null;
    }
    if (
      (typeof dropZoneElement == 'undefined' || dropZoneElement == null) &&
      typeof dragState.dropZoneElement != 'undefined' &&
      dragState.dropZoneElement != null
    ) {
      dragState.dropZoneElement.__zone.ToggleDropCues(
        false,
        __wpm.dragState.dropIndex,
        false
      );
      dragState.dropZoneElement = null;
...
function WebPartManager_ShowHelp(helpUrl, helpMode) {
  ...
  if (helpMode == 0 || helpMode == 1) {
    if (helpMode == 0) {
      var dialogInfo =
        'edge: Sunken; center: yes; help: no; resizable: yes; status: no';
...
function WebPartManager_ExportWebPart(exportUrl, warn, confirmOnly) {
  if (
    warn == true &&
    __wpmExportWarning.length > 0 &&
    this.personalizationScopeShared != true
  ) {
