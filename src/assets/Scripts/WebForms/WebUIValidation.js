...
function ValidatorConvert(op, dataType, val) {
  ...
  if (dataType == 'Integer') {
    ...
    if (
      m != null &&
      ((typeof m[2] != 'undefined' && m[2].length == 4) ||
        val.dateorder == 'ymd')
    ) {
      day = m[6];
      month = m[5];
      ...
      year =
        typeof m[5] != 'undefined' && m[5].length == 4
          ? m[5]
  ...
  (function ($) {
    ...
    function getAttributesWithPrefix(element, prefix) {
      ...
      for (i = 0; i < length; i++) {
        ...
        if (
          attribute.specified &&
          attribute.name.substr(0, prefixLength).toLowerCase() === prefix
        ) {
    ...
    function normalizeKey(key) {
      key = key.toLowerCase();
      return normalizedAttributes[key] === undefined
        ? key
        : normalizedAttributes[key];
    }
    function addValidationExpando(element) {
      var attributes = getAttributesWithPrefix(
        element,
    ...
    function parse(selector) {
      var length = parseSpecificAttribute(
        selector,
        dataValidationAttribute,
        Page_Validators
      );
      length += parseSpecificAttribute(
        selector,
        dataValidationSummaryAttribute,
