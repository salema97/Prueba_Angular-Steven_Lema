...
(function ($) {
  ...
  var DEFAULT_MIN = 0,
    ...
    validateAttr = function ($input, vattr, options) {
      var chk = isEmpty($input.data(vattr))
        ? $input.attr(vattr)
        : $input.data(vattr);
  ...
  Rating.prototype = {
    ...
    init: function (options) {
      ...
      self.$clearElement = isEmpty(options.clearElement)
        ? null
        : $(options.clearElement);
      self.$captionElement = isEmpty(options.captionElement)
        ? null
        : $(options.captionElement);
      if (self.$rating === undefined && self.$container === undefined) {
        self.$rating = $(document.createElement('div')).html(
      ...
      self.generateRating();
      self.$clear = !isEmpty(self.$clearElement)
        ? self.$clearElement
        : self.$container.find('.' + self.clearButtonBaseClass);
      self.$caption = !isEmpty(self.$captionElement)
        ? self.$captionElement
        : self.$container.find('.caption');
      self.setStars();
      self.listen();
      if (self.showClear) {
    ...
    generateRating: function () {
      ...
      if (self.glyphicon) {
        css +=
          (self.symbol === '\ue006' ? ' rating-gly-star' : ' rating-gly') +
          self.ratingClass;
      } else {
        css += isEmpty(self.ratingClass)
          ? ' rating-uni'
          : ' ' + self.ratingClass;
      }
    ...
    renderClear: function () {
      ...
      if (!isEmpty(self.$clearElement)) {
        ...
        self.$clearElement
          .attr({ title: self.clearButtonTitle })
    ...
    fetchCaption: function (rating) {
      ...
      cap = isEmpty(capVal)
        ? self.defaultCaption.replaceAll('{rating}', val)
        : capVal;
    ...
    return this.each(function () {
      ...
      if (!data) {
        $this.data(
          ...
          (data = new Rating(
            this,
            $.extend({}, $.fn.rating.defaults, options, $(this).data())
          ))
        );
      }
